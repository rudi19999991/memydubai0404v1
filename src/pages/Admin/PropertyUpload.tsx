...
import { Upload, Plus, X, Check, Building, LinkIcon } from "lucide-react";
...
const PropertyUpload = () => {
  ...
  const handleAddImageUrl = () => {
    if (currentImageUrl.trim() && !imageUrls.includes(currentImageUrl.trim())) {
      const newUrl = currentImageUrl.trim();
      setImageUrls((prev) => [...prev, newUrl]);
      setImagePreviewUrls((prev) => [...prev, newUrl]);
      setCurrentImageUrl("");
    }
  };

  const handleRemoveImage = (index: number) => {
    if (imageInputMethod === "file" && images[index]) {
      // Revoke the URL to prevent memory leaks for file uploads
            ...
            <div className="pt-4 border-t">
              ...
              <Tabs value={imageInputMethod} onValueChange={(value) => setImageInputMethod(value as "file" | "url")} className="w-full">
                <TabsList className="grid grid-cols-2 mb-4">
                  <TabsTrigger value="file">{translate("Upload Files")}</TabsTrigger>
                  <TabsTrigger value="url">{translate("Image URLs")}</TabsTrigger>
                </TabsList>
                    ...
                    <div key={index} className="relative">
                      <img
                        ...
                        onError={(e) => {
                          ...
                          target.src = "https://images.unsplash.com/photo-1582407947304-fd86f028f716?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2340&q=80";
                        }}
                      />
                      <Button
                        type="button"
